...
public class GridViewAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        View view;
        if (convertView == null) {
            view = LayoutInflater.from(CalculatorActivity.activity).inflate(layoutId, parent, false);
            viewHolder = new ViewHolder();
            viewHolder.title =  view.findViewById(R.id.text_item);
            if (viceText != null)
                viewHolder.vice =  view.findViewById(R.id.text_vice_item);
        ...
        if (layoutId == R.layout.button_operator) {
            if (deleteParam == null)
                deleteParam = CalculatorActivity.activity.delete.getLayoutParams();
            if (deleteParam.height != height / rows) {
                deleteParam.height = height / rows;
                CalculatorActivity.activity.delete.setLayoutParams(deleteParam);
            }
        }
