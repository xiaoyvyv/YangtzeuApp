...
public class CycleRulerView extends View {
    ...
    public int mainColor = 0xff1eb8f8;
    ...
    private void drawDisplay(Canvas canvas) {
        ...
        displayPaint1.setColor(mainColor);
        ...
        strokPaint.setColor(0xff999999);
        canvas.drawText(cm, width / 2 - cmWidth / 2, height * 3 / 5 + bounds1.height() / 2, displayPaint1);
    }
    ...
    private void onTouchStart(Coordinate coordinate) {
        getPoint(coordinate);
    }
    ...
    private void getPoint(Coordinate coordinate) {
        float mx = width / 2f;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN:
                onTouchStart(new Coordinate(event.getX(), event.getY()));
                break;
        }
        return true;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        x_paint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_OUT));
        x_paint.setStyle(Paint.Style.FILL);
        x_paint.setColor(mainColor);
        canvas.drawRect(new RectF(0, 0, width, height), x_paint);
        canvas.save();

        //画背景
        Paint bg_paint = new Paint();
        bg_paint.setColor(background);
        bg_paint.setAntiAlias(true);
        bg_paint.setStyle(Paint.Style.FILL);
        RectF rectF = new RectF(0, 0, width, height);
        canvas.drawRect(rectF, bg_paint);
        ...
        degreePaint.setColor(mainColor);
